...
int init() { return pixy_instance.init(); }

int change_prog(const char *program_name) {
  return pixy_instance.changeProg(program_name);
}

int get_frame_width() { return pixy_instance.frameWidth; }

int get_frame_height() { return pixy_instance.frameHeight; }
...
int ccc_get_blocks(int max_blocks, struct Block *blocks) {
  ...
  number_of_blocks_copied =
      (max_blocks >= blocks_available ? blocks_available : max_blocks);
...
void line_get_all_features() { pixy_instance.line.getAllFeatures(); }

void line_get_main_features() { pixy_instance.line.getMainFeatures(); }
...
int line_get_vectors(int max_vectors, struct Vector *vectors) {
  ...
  number_of_vectors_copied =
      (max_vectors >= vectors_available ? vectors_available : max_vectors);
...
int line_get_intersections(int max_intersections,
  ...
  number_of_intersections_copied =
      (max_intersections >= intersections_available ? intersections_available
                                                    : max_intersections);

  // Copy intersections //
  for (index = 0; index != number_of_intersections_copied; ++index) {
    memcpy(&intersections[index], &pixy_instance.line.intersections[index],
           sizeof(Intersection));
  }
...
int line_get_barcodes(int max_barcodes, struct Barcode *barcodes) {
  ...
  number_of_barcodes_copied =
      (max_barcodes >= barcodes_available ? barcodes_available : max_barcodes);

  // Copy barcodes //
  for (index = 0; index != number_of_barcodes_copied; ++index) {
    memcpy(&barcodes[index], &pixy_instance.line.barcodes[index],
           sizeof(Barcode));
  }
