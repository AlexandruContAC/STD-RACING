% module pixy
    ...
    % include "stdint.i" % include "carrays.i" % include "typemaps.i"
...
% inline % {
  ...
  /*!
    ...
    @param[in]   program_name  "color_connected_components"  Block detection
    program "line"                        Line feature detection program
  */
  extern int change_prog(const char *program_name);

  /*!
    @brief       Gets the Pixy sensor frame width.
    @return      Returns Pixy sensor width (in number of pixels) that is being
    sent to the host.
  */
  ...
  /*!
    ...
    @return      Returns Pixy sensor height (in number of pixels) that is being
    sent to the host.
  */
  ...
  */
  ...
  /*!
    @brief       Copy 'max_intersections' number of intersections to the address
    'intersections'.
    @param[in]   max_intersections  Maximum number of intersection objects that
    will be copied.
    @param[out]  intersections      Address to copy the intersection data.
    @return      Number of intersections copied to 'intersections'.
  */
  extern int line_get_intersections(int max_intersections,
                                    IntersectionArray *intersections);

  /*!
    @brief       Copy 'max_vectors' number of vectors to the address 'vectors'.
    @param[in]   max_vectors        Maximum number of vector objects that will
    be copied.
  ...
  */
  ...
  /*!
    @brief       Copy 'max_barcode' number of barcodes to the address
    'barcodes'.
    @param[in]   max_barcodes  Maximum number of barcode objects that will be
    copied.
...
% inline % {
  /*!
    @brief       Get pixel color components at the (X, Y) position on Pixy's
    sensor.
    @param[in]   X      X position of color pixel to get.
    @param[in]   Y      Y position of color pixel to get.
    @param[out]  Red    Memory address to write the Red color component value
    to.
    @param[out]  Green  Memory address to write the Green color component value
    to.
    @param[out]  Blue   Memory address to write the Blue color component value
    to.
  */
  extern void video_get_RGB(int X, int Y, uint8_t *Red, uint8_t *Green,
                            uint8_t *Blue);
  %
}
...
% extend Intersection{
      uint8_t getLineIndex(int i){return $self->m_intLines[i].m_index;
}
}

% extend Intersection {
  int16_t getLineAngle(int i) { return $self->m_intLines[i].m_angle; }
}