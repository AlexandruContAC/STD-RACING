...
LOG_MODULE_REGISTER(synapse_ethernet,
                    CONFIG_CEREBRI_SYNAPSE_ETHERNET_LOG_LEVEL);
...
typedef struct context_s {
  ...
  struct zros_sub sub_actuators, sub_battery_state, sub_estimator_odometry,
      sub_nav_sat_fix, sub_status;
  TinyFrame tf;
  int client;
...
static void synapse_ethernet_init(context_t *ctx) {
  ...
  zros_sub_init(&ctx->sub_estimator_odometry, &ctx->node,
                &topic_estimator_odometry, &ctx->estimator_odometry, 10);
  zros_sub_init(&ctx->sub_nav_sat_fix, &ctx->node, &topic_nav_sat_fix,
                &ctx->nav_sat_fix, 10);
...
static void ethernet_entry_point(context_t *ctx) {
  ...
  if (zsock_bind(ctx->serv, (struct sockaddr *)&ctx->bind_addr,
  ...
  // TF_AddTypeListener(&ctx->tf, SYNAPSE_ODOMETRY_TOPIC,
  // external_odometry_listener);
  TF_AddTypeListener(&ctx->tf, SYNAPSE_BEZIER_TRAJECTORY_TOPIC,
                     bezier_trajectory_listener);
  ...
  TF_AddTypeListener(&ctx->tf, SYNAPSE_CLOCK_OFFSET_TOPIC,
                     clock_offset_listener);

#ifdef CONFIG_CEREBRI_DREAM_HIL
  TF_AddTypeListener(&ctx->tf, SYNAPSE_BATTERY_STATE_TOPIC,
                     battery_state_listener);
  TF_AddTypeListener(&ctx->tf, SYNAPSE_IMU_TOPIC, imu_listener);
  TF_AddTypeListener(&ctx->tf, SYNAPSE_MAGNETIC_FIELD_TOPIC,
                     magnetic_field_listener);
  TF_AddTypeListener(&ctx->tf, SYNAPSE_NAV_SAT_FIX_TOPIC, nav_sat_fix_listener);
  TF_AddTypeListener(&ctx->tf, SYNAPSE_WHEEL_ODOMETRY_TOPIC,
                     wheel_odometry_listener);
#endif
  ...
  while (1) {
    ...
    while (!ctx->reconnect) {
      ...
      if (zros_sub_update_available(&ctx->sub_battery_state)) {
          ...
          TOPIC_PUBLISHER(&ctx->battery_state, synapse_msgs_BatteryState,
      SYNAPSE_BATTERY_STATE_TOPIC);
      }
      */

      if (zros_sub_update_available(&ctx->sub_nav_sat_fix)) {
        zros_sub_update(&ctx->sub_nav_sat_fix);
        TOPIC_PUBLISHER(&ctx->nav_sat_fix, synapse_msgs_NavSatFix,
                        SYNAPSE_NAV_SAT_FIX_TOPIC);
      }

      if (zros_sub_update_available(&ctx->sub_status)) {
        zros_sub_update(&ctx->sub_status);
        TOPIC_PUBLISHER(&ctx->status, synapse_msgs_Status,
                        SYNAPSE_STATUS_TOPIC);
      }

      if (zros_sub_update_available(&ctx->sub_estimator_odometry)) {
        zros_sub_update(&ctx->sub_estimator_odometry);
        TOPIC_PUBLISHER(&ctx->estimator_odometry, synapse_msgs_Odometry,
      ...
      if (do_polling) {
        ...
        if (rc >= 0) {
          if (fds.revents & ZSOCK_POLLIN) {
            int len =
                zsock_recv(ctx->client, ctx->rx1_buf, sizeof(ctx->rx1_buf), 0);
...
K_THREAD_DEFINE(synapse_ethernet, MY_STACK_SIZE, ethernet_entry_point, &g_ctx,
                NULL, NULL, MY_PRIORITY, 0, 0);
