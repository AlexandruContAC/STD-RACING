...
static int init(struct context *ctx) {
  ...
  ret = zros_sub_init(&ctx->sub_status, &ctx->node, &topic_status, &ctx->status,
                      1);
  if (ret < 0) {
  ...
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_BEZIER_TRAJECTORY_TOPIC,
                           bezier_trajectory_listener);
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_PIXY_VECTOR_TOPIC,
                           pixy_vector_listener);
  if (ret < 0)
    return ret;
  ...
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_CLOCK_OFFSET_TOPIC,
                           clock_offset_listener);
  if (ret < 0)
    return ret;
#ifdef CONFIG_CEREBRI_DREAM_HIL
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_BATTERY_STATE_TOPIC,
  ...
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_MAGNETIC_FIELD_TOPIC,
                           magnetic_field_listener);
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_NAV_SAT_FIX_TOPIC,
                           nav_sat_fix_listener);
  if (ret < 0)
    return ret;
  ret = TF_AddTypeListener(&ctx->tf, SYNAPSE_WHEEL_ODOMETRY_TOPIC,
...
static int start() {
  k_tid_t tid = k_thread_create(&g_my_thread_data, g_my_stack_area,
                                K_THREAD_STACK_SIZEOF(g_my_stack_area), run,
