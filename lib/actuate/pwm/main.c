...
void pwm_update(const synapse_msgs_Status* status, const synapse_msgs_Actuators* actuators)
{
    ...
    for (int i = 0; i < CONFIG_CEREBRI_ACTUATE_PWM_NUMBER; i++) {
        ...
        if (pwm.type == PWM_TYPE_NORMALIZED) {
            ...
            pwm_set_pulse_dt(&g_ctx.pwm_enable, PWM_USEC(input<0?0:50)); // pwm_enable is actually a direction selection 1=Forward 0=Backward
            //uint32_t output = (uint32_t)((pwm.slope * input) + pwm.intercept);//original implementation, not exactly good for b3rb hardware
            //ugly patch for b3rb hardware that only uses the CONFIG_CEREBRI_ACTUATE_PWM_PULSE_MAX_1 setting from cognipilot/ws/cerebri/app/b3rb/boards/mr_canhubk3.conf
            uint32_t output = (uint32_t)((pwm.max * fabs(input))); //requires adding math.h to includes 
