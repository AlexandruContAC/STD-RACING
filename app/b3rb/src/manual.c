...
static void b3rb_manual_entry_point(void* p0, void* p1, void* p2)
{
    ...
    while (true) {
        ...
        int rc = 0;
        rc = k_poll(events, ARRAY_SIZE(events), K_MSEC(10));
        if (rc != 0) {
        ...
        // to avoid confusion direction commands will be between -1 and 1 (relative) to straight which is 0
        double turn_angle = ctx->joy.axes[JOY_AXES_ROLL];
        ///double omega_fwd = ctx->max_velocity * ctx->joy.axes[JOY_AXES_THRUST] / ctx->wheel_radius;
        // to avoid confusion lets consider relative velocity to max phisically possible velocity 1 max forward, 0 stop, -1 max backward
        double omega_fwd = ctx->joy.axes[JOY_AXES_THRUST];
        static double steer_ref_f = 0, speed_ref_f = 0;
        static uint64_t last_ticks;  static bool do_init_static = true; if(do_init_static) { do_init_static=false; last_ticks = k_uptime_ticks();}
    
        filter_refs(omega_fwd, turn_angle, &speed_ref_f, &steer_ref_f, &last_ticks);
        //LOG_ERR("man_dt_ms %d", 
        b3rb_set_actuators(&ctx->actuators_manual, steer_ref_f, speed_ref_f);
