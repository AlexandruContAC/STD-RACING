...
// AUTO-ARM: wait for system to stabilize, then arm in AUTO mode (Pixy line-following)
    LOG_WRN("auto-arm: waiting 3 seconds before arming...");
    k_sleep(K_MSEC(3000));
    ctx->status.mode = synapse_msgs_Status_Mode_MODE_AUTO;
    ctx->status.arming = synapse_msgs_Status_Arming_ARMING_ARMED;
    LOG_WRN("auto-arm: ARMED in AUTO mode (Pixy line-following)");
